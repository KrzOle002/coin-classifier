...
def augment_gray(gray: np.ndarray) -> list[np.ndarray]:
    ...
    variants.append(cv2.flip(gray, 1))
    return variants
...
def prepare_data(verbose: bool = True):
    ...
    # Augmentacja tylko zbioru treningowego
    X_train_list, y_train_list = [], []
    for i in idx_train:
        for variant in augment_gray(raw_X[i]):
            blur  = cv2.GaussianBlur(variant, (BLUR_K, BLUR_K), 0)
            edges = cv2.Canny(blur, CANNY_LOW, CANNY_HIGH)
            X_train_list.append(extract_features(edges))
            y_train_list.append(raw_y[i])

    # Test bez augmentacji
    X_test_list, y_test_list = [], []
    for i in idx_test:
        blur  = cv2.GaussianBlur(raw_X[i], (BLUR_K, BLUR_K), 0)
        edges = cv2.Canny(blur, CANNY_LOW, CANNY_HIGH)
        X_test_list.append(extract_features(edges))
        y_test_list.append(raw_y[i])
    ...
    if verbose:
        print(f"  Po augmentacji: trening={len(y_train)} (7x), test={len(y_test)}")
